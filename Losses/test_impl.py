...
def value_and_compute_gradient_and_update(
    ...
    # # - Debug
    # v_num, grads_num = numerical_value_and_grad(
    #     params,
    #     net,
    #     X,
    #     y,
    ...
    #     rng_key
    # )
    # v_jax, grads_jax = value_and_grad(adversarial_loss)(
    #     params,
    #     net,
    #     X,
    #     y,
    ...
    #     rng_key
    # )
    # print("Value jax %.4f numerical: %.4f" % (v_jax,v_num))
    # for k in grads_jax:
    #     assert onp.allclose(grads_jax[k], grads_num[k], atol=1e-2)

    return value, opt_update(batch_id, grads, opt_state), grads
...
if __name__ == '__main__':
    ...
    cnn_torch = TorchCNN().to(device)
    cnn_torch.apply(init_weights)

    # - Get the parameters of torch
    name_swap = {
        "conv1.weight": "K1",
        "conv2.weight": "K2",
        "conv1.bias": "CB1",
    ...
    params_torch = {}
    for name, val in cnn_torch.named_parameters():
        w = val.data.cpu().numpy()
        if "linear" in name:
            w = w.T
        params_torch[name_swap[name]] = w
    params_torch["CB1"] = jnp.reshape(params_torch["CB1"], newshape=(1,64,1,1))
    params_torch["CB2"] = jnp.reshape(params_torch["CB2"], newshape=(1,64,1,1))

    opt_init, opt_update, get_params = optimizers.adam(LR, 0.9, 0.999, 1e-08)
    opt_state = opt_init(params_torch)

    # - Create adam instance for torch
    optimizer = torch.optim.Adam(cnn_torch.parameters(), lr=LR)
    ...
    for epoch_id in range(N_EPOCHS):
        ...
        for idx,(X,y) in enumerate(train_dataloader):
            ...
            # out_jax = cnn_jax.call(X_jax, **params_torch)
            # out_torch = cnn_torch(X)
            # assert onp.allclose(out_jax, out_torch.detach().cpu().numpy(), atol=1e-3)

            torch_robustness_loss = adv_loss.compute_gradient_and_backward(
                model=cnn_torch,
                X=X,
                y=y
            )

            # TODO remove return of grads
            loss, opt_state, grads = value_and_compute_gradient_and_update(
                batch_id=idx,
                X=X_jax,
                y=y_jax,
                opt_state=opt_state,
            ...
            print(loss, torch_robustness_loss)
            for name, v in cnn_torch.named_parameters():
                grad_v = deepcopy(v.grad)
                if "linear" in name:
                    grad_v = grad_v.T
                assert onp.allclose(
                    grad_v.cpu().numpy().reshape((-1,)),
                    grads[name_swap[name]].reshape((-1,)),
                    atol=1e-4
                )
