...
class pcm_sim_robustness:
    ...
    @staticmethod
    def train_grid():
        resnet_awp = [Resnet.make()]
        resnet_awp = configure(resnet_awp, {"n_attack_steps":3, "batch_size":256, "clipping_alpha":2.0,
                                    "start_epoch":60, "pretrained":True, "epochs":200, "gamma":gamma})
        resnet_awp = split(resnet_awp, "eps_pga", eps_pgas)
        resnet_awp = split(resnet_awp, "attack_size_mismatch", attack_size_mismatches)
        ...
        resnet = split(resnet + baseline + resnet_awp, "seed", seeds)
        return resnet
    ...
    def visualize():
        ...
        blue_vals = np.linspace(242. / 255., 0.3 ,num=len(beta_robustnesses))
        eps_colors = np.linspace(242. / 255., 0.3 ,num=len(eps_pgas))
        ...
        for i,eta_train in enumerate(eta_trains):
            ...
            cmap = ListedColormap([(0.3,g,1.0) for g in blue_vals])
            ...
            norm_awp = mpl.colors.Normalize(vmin=eps_pgas[0],vmax=eps_pgas[-1])
            cmap_awp = ListedColormap([(0.0,g,0.01) for g in eps_colors])
            cbar_awp = fig.colorbar(cm.ScalarMappable(norm=norm_awp, cmap=cmap_awp), ax=axes[i,0], aspect=30, location="left")
            cbar_awp.ax.tick_params(labelsize=6)
            cbar_awp.outline.set_visible(False)
            cbar_awp.set_label(r"$\epsilon_{pga}$")
            cbar_awp.set_ticks(np.linspace(eps_pgas[0],eps_pgas[-1],len(eps_pgas)))
            cbar_awp.set_ticklabels([str(ep) for ep in eps_pgas])
